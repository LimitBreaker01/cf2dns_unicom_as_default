...
class AliApi():
    ...
    def get_record(self, domain, length, sub_domain, record_type):
        ...
        request.set_accept_format(rc_format)
        result = clt.do_action(request).decode('utf-8').replace('DomainRecords', 'data', 1).replace('Record', 'records', 1).replace('RecordId', 'id').replace('Value', 'value').replace('Line', 'line').replace('telecom', '电信').replace('default', '联通').replace('mobile', '移动')
        result = json.JSONDecoder().decode(result)
        return result
